<!-------------------------------Footer ------------------------------------------>
  ...
  <script>
    ...
    $(document).ready(function() {
      // Function to check session status and update navbar
      function updateNavbar() {
        $.ajax({
          url: 'ajax/session_status.php',
          type: 'GET',
          dataType: 'json',
          success: function(data) {
            ...
            if (data.logged_in) {
              ...
              authButtons.html(`
                <button type="button" class="btn btn-outline-dark shadow me-lg-3 me-2" onclick="profileClicked()">Profile</button>
                <button type="button" class="btn btn-outline-dark shadow me-lg-3 me-2" onclick="logout()">Logout</button>
              `);
            } else {
              // Show Login and Register buttons
              authButtons.html(`
                <button type="button" class="btn btn-outline-dark shadow me-lg-3 me-2" data-bs-toggle="modal" data-bs-target="#loginmodal">Login</button>
                <button type="button" class="btn btn-outline-dark shadow me-lg-3 me-2" data-bs-toggle="modal" data-bs-target="#registermodal">Register</button>
              `);
            }
          }
        });
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response === 'success') {
              ...
              $('#alert-box').html('<div class="alert alert-success">Login successful!</div>');
              updateNavbar(); // Update the navbar after successful login
            } else if (response === 'invalid_pass') {
              $('#alert-box').html('<div class="alert alert-danger">Incorrect password.</div>');
            } else if (response === 'invalid_credential') {
              $('#alert-box').html('<div class="alert alert-danger">Email not found.</div>');
            } else {
      ...
      function logout() {
        $.ajax({
          ...
          type: 'POST',
          success: function(response) {
            updateNavbar(); // Refresh the navbar after logout
          }
        });
      }

      // Attach the logout function to the global scope for use in HTML
      window.logout = logout;

      // Sample profile button click function
      window.profileClicked = function() {
        window.location.href = "profile.php"; // Redirect to profile page
      };
    });

    // Function to show alert when Profile button is clicked
    function profileClicked() {
      window.location.href = "profile.php";
    }

    // Function to redirect to logout.php when Logout button is clicked
    function logout() {
      $.ajax({
          url: 'logout.php?nocache=' + new Date().getTime(),
          method: 'POST',
          success: function(response) {
